...
class Controller(private val service: Service,
    ...
    @ApiResponses(value = [
        ApiResponse(responseCode = "200", description = DATA_FOUND, content = [
            (Content(mediaType = MEDIA_TYPE, array = (
                    ArraySchema(schema = Schema(implementation = DeliveryScheduleModel::class)))))]),
    ...
    fun get(@RequestParam(required = false) storeNumber:Long?,
        ...
        service.getByQueryParam(storeNumber, deliveryStreamNumber,deliveryStreamName,
                schemaName,startDate,endDate,notes).forEach { model -> records.add(model.data)}
        print(records)
    ...
    @ApiResponses(value = [
        ApiResponse(responseCode = "200", description = DATA_FOUND, content = [Content(schema = Schema(implementation = DeliveryScheduleModel::class))]),
        ApiResponse(responseCode = "400", description = BAD_REQUEST, content = [Content()]),
        ApiResponse(responseCode = "404", description = NO_DATA_FOUND, content = [Content()])]
    )
